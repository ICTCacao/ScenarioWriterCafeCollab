...
正式なライセンス条文は英文の [LICENSE](LICENSE) です。その免責条項の趣旨を日本語で説明すると、次のとおりです。

- このソフトウェアは **現状のまま（AS IS）** 提供します。動作・品質・特定の目的への適合性・権利侵害がないことなど、**いかなる保証もしません**。
- このソフトウェアの使用、または使用できなかったことによって生じた **いかなる損害（データの消失、業務の中断、逸失利益などを含む）についても、作者および著作権者は責任を負いません**。契約・不法行為・その他どのような法的根拠によっても同様です。
- 台本のデータは利用者の責任で保管してください。SQLite の DB ファイル（`sw_config/swdata.sqlite`）のコピーや、「ダウンロード」で作った台本ファイルを **こまめに控えておく** ことを勧めます。
- 公開サーバに設置する場合のセキュリティ（HTTPS、パスワード管理、`sw_config/` へのアクセス制限、`swSetup.php` の削除など）は、設置者の責任で行ってください。
