<?php
// -----------------------------------------------------------
//
// Copyright (C) 2026 ICTCacao Released under the MIT License (see LICENSE).
//
//     シナリオをDOCXでダウンロード
//
//     swScenarioDownloadDocx.php
// -----------------------------------------------------------
// ------------------------------------------------------------------------------
  //定数読み込み
  include_once("./sw_config/swConstant.php");
  //DB接続ｸﾗｽの初期化
  include_once("./include/ConnectMySQL.php");
// ------------------------------------------------------------------------------
  //処理時間無制限
  $ret = set_time_limit(0);

  //共通関数をｲﾝｸﾙｰﾄﾞ
  include_once("./include/swFunc.php");
  include_once("./include/swFuncScenarioXML.php");

  //ﾌｫｰﾑのﾃﾞｰﾀを読み込む
  fncGetPostItems($mySqlConnObj);

  //ｾｯｼｮﾝ管理
  include_once("./include/swAccept.php");
  include_once("./include/swCheckAdmin.php");
  //共同執筆: 権限ガード（閲覧）
  $swCollabNeed = 'read';
  include_once("./include/swCollabGuard.php");

  //シナリオIDが無いまま進むと ./tmp/ 自体を複写先にして最後に丸ごと削除してしまうので止める
  if($fdtScenarioId == ''){ exit('シナリオIDがありません'); }

  //MainProcedure
  fncMainProc($mySqlConnObj);
  //ﾌｧｲﾙ出力
  fncScenarioDownload();
  exit();

// ------------------------------------------------------------------------------
//      POSTされた要素を取得
//          fncGetPostItems($mySqlConnObj)
// ------------------------------------------------------------------------------
function fncGetPostItems($mySqlConnObj){
  //共通global変数
  global  $fdtUserLoginId,$fdtUserLoginDate;
  global  $fdtScenarioId,$fdtUserName,$fdtScenarioTitle;

  global  $fdtCharacterCode,$fdtLineFeedCode;
  global  $fdtTemplateId;
  global  $fdtTemplateName;

  global  $fdtWriterName,$fdtWriterId,$fdtVersion,$fdtDate;
  global  $fdtWriterAddress,$fdtWriterPhoneNo,$fdtWriterEMail;
  global  $fdtDateYY,$fdtDateMM,$fdtDateDD;

  //POSTされた要素を取得
  $fdtUserLoginId = swFunc_GetPostData('fdtUserLoginId');          //USERログインID
  $fdtUserLoginDate = swFunc_GetPostData('fdtUserLoginDate');      //USERログイン日時
  $fdtScenarioId = swFunc_GetPostData('fdtScenarioId');      //シナリオID

  $fdtCharacterCode = swFunc_GetPostData('fdtCharacterCode');    //文字コード
  $fdtLineFeedCode = swFunc_GetPostData('fdtLineFeedCode');      //改行コード

  $fdtTemplateId = swFunc_GetPostData('fdtTemplateId');      //テンプレートID

  $fdtWriterName = swFunc_GetPostData('fdtWriterName');        //作者名
  $fdtWriterId = swFunc_GetPostData('fdtWriterId');            //脚本協会登録番号等
  $fdtVersion = swFunc_GetPostData('fdtVersion');              //草稿バージョン等
  $fdtWriterAddress = swFunc_GetPostData('fdtWriterAddress'); //住所
  $fdtWriterPhoneNo = swFunc_GetPostData('fdtWriterPhoneNo'); //電話番号
  $fdtWriterEMail = swFunc_GetPostData('fdtWriterEMail');     //電子メール
  $fdtDate = $fdtDateYY.'/'.$fdtDateMM.'/'.$fdtDateDD;
  if ($fdtDate == "//"){$fdtDate = "";}
  if ($fdtDate == "0000/00/00"){$fdtDate = "";}


  //$fdtTemplateName
  switch($fdtTemplateId){
    case 'A4TP':
      $fdtTemplateName = 'scenario_template_tate_A4portrait';
      break;
    case 'A4YP':
      $fdtTemplateName = 'scenario_template_yoko_A4portrait';
      break;
    case 'A4TL':
      $fdtTemplateName = 'scenario_template_tate_A4landscape';
      break;
    default:
      $fdtTemplateName = '';
      break;
  }
}//end function


// ------------------------------------------------------------------------------
//      MAIN PROCEDURE
//          fncMainProc($mySqlConnObj)
// ------------------------------------------------------------------------------
function fncMainProc($mySqlConnObj){
  global  $documentXML,$ScenarioTitle;
  //共通global変数
  global  $fdtUserLoginId,$fdtUserLoginDate;
  global  $fdtScenarioId;
  //USER STYLE
  global  $UserOptionStyleArray;
  //シナリオのキャラクタ配列
  global  $ScenarioCharacterArray;

  global  $documentXML;

  global  $fdtCharacterLength,$fdtBodyLength;
  global  $fdtTemplateName,$fdtTemplateId;

  global  $fdtWriterName,$fdtWriterId,$fdtVersion,$fdtDate;
  global  $fdtWriterAddress,$fdtWriterPhoneNo,$fdtWriterEMail;
  global  $fdtDateYY,$fdtDateMM,$fdtDateDD;


  //テンプレートを複写
  $src = './Scenario_Template/'.$fdtTemplateName;
  $dst = './tmp/'.$fdtScenarioId;
  if(!is_dir('./tmp')){ mkdir('./tmp', 0777, true); }   //一時ﾌｫﾙﾀﾞが無ければ作る
  fncRecursiveCopyDir($src, $dst);


  //------------------------------------------------------------
  //  複写したフォルダ構成
  //------------------------------------------------------------
  //  tmp/ScenarioId($dst)
  //        ├ _rels
  //        │    └ .rels
  //        │
  //        ├ docProps
  //        │    ├ app.xml
  //        │    └ core.xml  --> User名を変更して保存
  //        │
  //        ├ customXml
  //        │    ├ _rels
  //        │    │  └ item1.xml.rels
  //        │    ├ item1.xml
  //        │    └ itemProps1.xml
  //        │
  //        ├ word
  //        │    ├ _rels
  //        │    │  ├ document.xml.rels
  //        │    │  └ settings.xml.rels
  //        │    ├ theme
  //        │    │  └ theme1.xml
  //        │    ├ document.xml    --> シナリオ本体を編集して保存
  //        │    ├ endnotes.xml
  //        │    ├ fontTable.xml
  //        │    ├ footer1.xml
  //        │    ├ footer2.xml    --> フッター部のタイトルを編集して保存
  //        │    ├ footnotes.xml
  //        │    ├ numbering.xml
  //        │    ├ settings.xml
  //        │    ├ styles.xml
  //        │    ├ stylesWithEffects.xml
  //        │    └ webSettings.xml
  //        │
  //        └ [Content_Types].xml
  //------------------------------------------------------------

  //ﾛｸﾞｲﾝ情報からﾕｰｻﾞ情報を取得
  //ﾃﾞｰﾀ管理ｸﾗｽ ﾛｸﾞｲﾝ情報
  include_once("./class/clsSwUserLoginInfo.php");
  $clsSwUserLoginInfo = new clsSwUserLoginInfo();
  //ﾃﾞｰﾀ管理ｸﾗｽの初期化
  $clsSwUserLoginInfo->clsSwUserLoginInfoInit($mySqlConnObj,$fdtUserLoginId);
  //ﾌﾟﾛﾊﾟﾃｨGet
  $UserId = $clsSwUserLoginInfo->clsSwUserLoginInfoGetUserId(); //USER_ID


  //ﾃﾞｰﾀ管理ｸﾗｽ ﾕｰｻﾞ情報
  include_once("./class/clsSwUser.php");
  $clsSwUser = new clsSwUser();
  //ﾃﾞｰﾀ管理ｸﾗｽの初期化
  $clsSwUser->clsSwUserInit($mySqlConnObj,$UserId);
  //ﾌﾟﾛﾊﾟﾃｨGet
  $fdtUserName = $clsSwUser->clsSwUserGetUserName();                //ユーザー名

  //ﾃﾞｰﾀ管理ｸﾗｽ
  include_once("./class/clsSwUserOption.php");
  $clsSwUserOption = new clsSwUserOption();
  //USER STYLEを配列化
  $UserOptionStyleArray = $clsSwUserOption->fncGetUserOptionStyle($mySqlConnObj,$UserId);
  //ﾃﾞｰﾀ管理ｸﾗｽ ﾕｰｻﾞ情報
  include_once("./class/clsSwUserOptionSetting.php");
  $clsSwUserOptionSetting = new clsSwUserOptionSetting();
  //ﾃﾞｰﾀ管理ｸﾗｽの初期化
  $UserOptionSettingCnt = $clsSwUserOptionSetting->clsSwUserOptionSettingInit($mySqlConnObj,$UserId);
  //ﾌﾟﾛﾊﾟﾃｨGet
  if($UserOptionSettingCnt == '0'){
    $fdtCharacterLength = '8';
    $fdtBodyLength = '20';
  }else{
    $fdtCharacterLength = $clsSwUserOptionSetting->clsSwUserOptionSettingGetCharacterLength();  //キャラクタ部の文字数
    $fdtBodyLength = $clsSwUserOptionSetting->clsSwUserOptionSettingGetBodyLength();            //シナリオ本文の文字数
  }



  //シナリオIDからシナリオ情報を取得
  //ﾃﾞｰﾀ管理ｸﾗｽ
  include_once("./class/clsSwScenario.php");
  $clsSwScenario = new clsSwScenario();
  //ﾃﾞｰﾀ管理ｸﾗｽの初期化
  $clsSwScenario->clsSwScenarioInit($mySqlConnObj,$fdtScenarioId);
  //ﾌﾟﾛﾊﾟﾃｨGet
  $ScenarioTitle = $clsSwScenario->clsSwScenarioGetScenarioTitle();      //タイトル
  $ScenarioSubtitle = $clsSwScenario->clsSwScenarioGetScenarioSubtitle();//サブタイトル
  $ScenarioWriterName = $clsSwScenario->clsSwScenarioGetScenarioWriterName();//作者名
  //ｻﾆﾀｲｽﾞ
  //$ScenarioTitle = swFunc_SanitizeStrings($ScenarioTitle);
  //$ScenarioSubtitle = swFunc_SanitizeStrings($ScenarioSubtitle);
  //$ScenarioWriterName = swFunc_SanitizeStrings($ScenarioWriterName);


  //ﾃﾞｰﾀ管理ｸﾗｽ
  include_once("./class/clsSwCharacter.php");
  $clsSwCharacter = new clsSwCharacter();
  //キャラクタをIDで連想配列にする
  $ScenarioCharacterArray = $clsSwCharacter->fncGetScenarioCharacter($mySqlConnObj,$fdtScenarioId);


  //---------------------------------------------------------------------------
    //core.xml上書き保存
    swFuncScenarioFilePutCoreXML($dst,$fdtUserName,$fdtTemplateId);

    //header1.xml上書き保存
    swFuncScenarioFilePutHeader1XML($dst,$ScenarioTitle,$fdtTemplateId);
  //---------------------------------------------------------------------------
    //footer2.xml上書き保存
    swFuncScenarioFilePutFooter2XML($dst,$ScenarioTitle,$fdtTemplateId);
  //---------------------------------------------------------------------------


  //XML編集開始 document.xml HEADER
  $documentXML = swFuncDocumentXMLHeader($fdtTemplateId);

  //表紙
  $ScenarioWriterName = $fdtWriterName;
  $Date = swFunc_SWHenkan($fdtDate);
  $Version = $fdtVersion;
  $WriterId =  $fdtWriterId;
  $Address = $fdtWriterAddress;
  $PhoneNo = $fdtWriterPhoneNo;
  $Email = $fdtWriterEMail;

  $documentXML .= swFuncDocumentXMLCover($ScenarioTitle,$ScenarioSubtitle,$Date,$Version,
                                            $ScenarioWriterName,$WriterId,$Address,$PhoneNo,$Email,$fdtTemplateId);


  //登場人物表
  $documentXML .= swFuncDocumentXMLCharacterListTitle($ScenarioTitle,$fdtTemplateId);
  //登場人物を検索
  $strCharacterSQL = <<<END_OF_SQL

    SELECT * FROM SW_CHARACTER
      WHERE SCENARIO_ID = :ScenarioId
          ORDER BY CHARACTER_ORDER_NO;
END_OF_SQL;
  $chara_stmt = $mySqlConnObj->prepare($strCharacterSQL);
  $chara_stmt->setFetchMode(PDO::FETCH_ASSOC);
  //パラメータのセット
  $chara_stmt->bindParam(':ScenarioId', $fdtScenarioId, PDO::PARAM_INT);
  $chara_stmt->execute();
  while($myCharaRow = $chara_stmt -> fetch(PDO::FETCH_ASSOC)) {
    $CharacterName = $myCharaRow['CHARACTER_NAME'];
    //サニタイズ
    //$CharacterName = swFunc_SanitizeStrings($CharacterName);
    $CharacterChara = $myCharaRow['CHARACTER_CHARA'];
    //<br>を改行にする
    $CharacterChara = str_replace("<br>","<w:br />",(string)$CharacterChara);   //PHP8.1: DBのNULLをそのまま渡すと Deprecated
    //キャラクター詳細
    if($CharacterChara == ''){$CharacterChara = $CharacterName;}

    $documentXML .= swFuncDocumentXMLCharacterList($CharacterName,$CharacterChara,$fdtTemplateId);
  }



  //シノプシス
  include_once("./class/clsSwSynopsis.php");
  $clsSwSynopsis = new clsSwSynopsis();
  //ﾃﾞｰﾀ管理ｸﾗｽの初期化
  $clsSwSynopsis->clsSwSynopsisInitScenarioId($mySqlConnObj,$fdtScenarioId);
  $fdtSynopsis = $clsSwSynopsis->clsSwSynopsisGetSynopsis();                //SYNOPSIS
  $fdtSynopsis = str_replace("<br>","\n",$fdtSynopsis);
  //シノプシスを全角にする
  $fdtSynopsis = mb_convert_kana($fdtSynopsis, 'KVRN');
  //改行処理
  $fdtSynopsis = str_replace("\n","<w:br />",$fdtSynopsis);

  //XML書き出し
  $documentXML .= swFuncDocumentXMLSynopsis($ScenarioTitle,$fdtSynopsis,$fdtTemplateId);


  //場面を検索
  $strSceneSQL = <<<END_OF_SQL

    SELECT * FROM SW_SCENE
      WHERE SCENARIO_ID = :ScenarioId
          ORDER BY SCENE_ORDER_NO;
END_OF_SQL;
  $scene_stmt = $mySqlConnObj->prepare($strSceneSQL);
  $scene_stmt->setFetchMode(PDO::FETCH_ASSOC);
  //パラメータのセット
  $scene_stmt->bindParam(':ScenarioId', $fdtScenarioId, PDO::PARAM_INT);
  $scene_stmt->execute();
  while($mySceneRow = $scene_stmt -> fetch(PDO::FETCH_ASSOC)) {
    $SceneId = $mySceneRow['SCENE_ID'];
    $SceneName = $mySceneRow['SCENE_NAME'];
    $SceneDescription = $mySceneRow['SCENE_DESCRIPTION'];
    //<br>を改行にする
    $SceneDescription = str_replace("<br>","<w:br />",(string)$SceneDescription);   //PHP8.1: DBのNULLをそのまま渡すと Deprecated
    //柱
    $documentXML .= swFuncDocumentXMLScene($SceneName,$SceneDescription,$fdtTemplateId);

    //場面のシナリオを検索
    $strScenarioSQL = <<<END_OF_SQL

        SELECT * FROM SW_SCENARIO_LINES
          WHERE SCENE_ID = '$SceneId'
              ORDER BY SCENARIO_LINES_ORDER_NO;
END_OF_SQL;
    $scenario_stmt = $mySqlConnObj->prepare($strScenarioSQL);
    $scenario_stmt->setFetchMode(PDO::FETCH_ASSOC);
    $scenario_stmt->execute();
    while($myScenarioRow = $scenario_stmt -> fetch(PDO::FETCH_ASSOC)) {
        $ScenarioLinesId = $myScenarioRow['SCENARIO_LINES_ID'];
        $ScenarioLinesOrderNo = $myScenarioRow['SCENARIO_LINES_ORDER_NO'];
        $ScenarioType = $myScenarioRow['SCENARIO_TYPE'];
        $CharacterId = $myScenarioRow['CHARACTER_ID'];
        $ScenarioLines = $myScenarioRow['SCENARIO_LINES'];

        //<br> -> \n 改行を置き換える
        $ScenarioLines = str_replace("<br>","\n",$ScenarioLines);
        //シナリオを全角にする
        $ScenarioLines = swFunc_ConvertKana($ScenarioLines, 'KVRNAZ');
        //サニタイズ
        //$ScenarioLines = swFunc_SanitizeStrings($ScenarioLines);
        //XML改行に置き換える
        $ScenarioLines = str_replace("\n","<w:br />",$ScenarioLines);

        //USER STYLEを反映させる
        if(isset($UserOptionStyleArray[$ScenarioType])){
          $StyleArray = explode(',',$UserOptionStyleArray[$ScenarioType]);
          $UserOptionStyleFontSize = '12';
          $UserOptionStyleColor = '#000';
          $UserOptionStyleIndent = '0';
          $UserOptionStyleStr = '';
          $UserOptionStyleWord = '0';
          if(isset($StyleArray[0])){$UserOptionStyleFontSize = $StyleArray[0];}
          if(isset($StyleArray[1])){$UserOptionStyleColor = $StyleArray[1];}
          if(isset($StyleArray[2])){$UserOptionStyleIndent = $StyleArray[2];}
          if(isset($StyleArray[3])){$UserOptionStyleStr = $StyleArray[3];}
          if(isset($StyleArray[4])){$UserOptionStyleWord = $StyleArray[4];}

          switch($UserOptionStyleWord){
            case '1'://登場人物名表示&台詞を「」で囲む
                $ScenarioLines = ''.swFunc_RemoveLastStr($ScenarioLines,'。').'';
              break;
            case '2'://登場人物名表示&台詞を「」で囲まない
              break;
            case '3'://登場人物名非表示&台詞を「」で囲む
                $ScenarioLines = ''.swFunc_RemoveLastStr($ScenarioLines,'。').'';
              break;
            default://登場人物名非表示&台詞を「」で囲まない
              break;
          }
        }else{
          //ScenarioTypeで処理を分岐
          switch($ScenarioType){
            case '1':    //台詞
              $ScenarioLines = ''.swFunc_RemoveLastStr($ScenarioLines,'。').'';
              $UserOptionStyleIndent = 0;
              $UserOptionStyleStr = '';
              break;
            case '2':    //ト書
              $UserOptionStyleIndent = 3;
              $UserOptionStyleStr = '';
              break;
            case '3':    //歌詞
              $UserOptionStyleIndent = 3;
              $UserOptionStyleStr = '歌';
              break;
            case '4':    //ナレーション
              $ScenarioLines = ''.swFunc_RemoveLastStr($ScenarioLines,'。').'';
              $UserOptionStyleIndent = 0;
              $UserOptionStyleStr = 'NA';
              break;
            case '5':    //モノローグ
              $ScenarioLines = ''.swFunc_RemoveLastStr($ScenarioLines,'。').'';
              $UserOptionStyleIndent = 0;
              $UserOptionStyleStr = 'M';
              break;
            case '6':    //テロップ
              $UserOptionStyleIndent = 3;
              $UserOptionStyleStr = 'T';
              break;
            case '7':    //演技指示
              $UserOptionStyleIndent = 3;
              $UserOptionStyleStr = '';
              break;
            case '8':    //音響指示
              $UserOptionStyleIndent = 3;
              $UserOptionStyleStr = 'SE';
              break;
            case '9':    //照明指示
              $UserOptionStyleIndent = 3;
              $UserOptionStyleStr = 'L';
              break;
            default:
              break;
          }//end of switch
        }//end if

        //登場人物名を配列から取得
        if(isset($ScenarioCharacterArray[$CharacterId])){
          $CharacterName = $ScenarioCharacterArray[$CharacterId];
          //サニタイズ
          //$CharacterName = swFunc_SanitizeStrings($CharacterName);
        }else{
          $CharacterName = '';
        }
        //キャラクター名 半角文字を全角文字にする
        $CharacterName = mb_convert_kana($CharacterName, 'KVRN');
        $UserOptionStyleStr = mb_convert_kana($UserOptionStyleStr, 'KVRN');
        //キャラクター名部分
        if($CharacterName == ''){
          if($UserOptionStyleStr == ''){
            //空白
            $CharacterName = '';
          }else{
            //スタイル略語
            $CharacterName = $UserOptionStyleStr;
          }
        }else{
          if($UserOptionStyleStr == ''){
            //キャラクター名
            $CharacterName = $CharacterName;
          }else{
            //キャラクター名+スタイル略語
            $CharacterName = $CharacterName.'　'.$UserOptionStyleStr;
          }
        }

        //シナリオ出力
        if($UserOptionStyleIndent > '0'){
          $CharacterName = '　　　　　　　　　　'.$CharacterName;
          $CharacterName = swFunc_SubstrR($CharacterName,$fdtCharacterLength);
          $documentXML .= swFuncDocumentXMLTogaki($CharacterName,$ScenarioLines,$fdtTemplateId);
        }else{
          $documentXML .= swFuncDocumentXMLScenarioLine($CharacterName,$ScenarioLines,$fdtTemplateId);
        }

    }//end while  ScenarioLines
  }//end while  Scene

  //body end
  $documentXML .= swFuncDocumentXMLBodyEnd($fdtTemplateId);



    //OS 判定
  if (DIRECTORY_SEPARATOR == '\\') {
    $osInfo = 'Windows';
  } else {
    $osInfo = 'NotWindows';
  }
  if ($osInfo == 'NotWindows'){
    chmod($dst, 0777);
  }
  //作成するﾌｧｲﾙ名の指定
  $file_name = $dst.'/word/document.xml';
  //ﾌｧｲﾙが存在したら
  //ﾌｧｲﾙ作成
  file_put_contents($file_name, $documentXML);
  //Windows以外は属性変更
  if ($osInfo == 'NotWindows'){
    //ﾌｧｲﾙの属性変更
    chmod( $file_name, 0755 );
  }
}//end function

// ------------------------------------------------------------------------------
//      シナリオデータ　ダウンロード
//          fncScenarioDownload()
// ------------------------------------------------------------------------------
function fncScenarioDownload(){
  global  $fdtScenarioId;
  global  $documentXML,$ScenarioTitle;
  global  $fdtCharacterCode,$fdtLineFeedCode;
  global  $fdtTemplateId;
//  //文字コード
//  if($fdtCharacterCode != 'UTF-8'){
//    $documentXML = mb_convert_encoding($documentXML, $fdtCharacterCode, 'UTF-8');
//  }
//  //改行コード
//  if($fdtLineFeedCode != "LF"){
//    switch($fdtLineFeedCode){
//      case 'LF':
//        $fdtLineFeedCode = "\n";
//        break;
//      case 'CR':
//        $fdtLineFeedCode = "\r";
//        break;
//      case 'CRLF':
//        $fdtLineFeedCode = "\r\n";
//        break;
//      default:
//        break;
//    }//end of switch
//    $documentXML = str_replace("\n",$fdtLineFeedCode,$documentXML);
//  }

  //ﾌｧｲﾙ名
  $filename = $ScenarioTitle . ".docx";
  $filename = mb_convert_encoding($filename, 'Shift_JIS', 'UTF-8');

  // Zipクラスロード
  $zip = new ZipArchive();
  //Zipファイル名
  $zipFileName = 'swc'.$fdtScenarioId.'.zip';
  // Zipファイル一時保存ディレクトリ
  $zipTmpDir = './tmp/'.$fdtScenarioId."/";
  // Zipファイルオープン
  $result = $zip->open($zipTmpDir.$zipFileName, ZIPARCHIVE::CREATE | ZIPARCHIVE::OVERWRITE);
  if($result === true){
    $zip->addEmptyDir('_rels');
    $zip->addFile($zipTmpDir.'_rels/.rels', '_rels/.rels');

    if($fdtTemplateId == 'A4TP'){
      $zip->addEmptyDir('customXml');
      $zip->addEmptyDir('customXml/_rels');
      $zip->addFile($zipTmpDir.'customXml/_rels/item1.xml.rels', 'customXml/_rels/document.xml.rels');
      $zip->addFile($zipTmpDir.'customXml/item1.xml', 'customXml/item1.xml');
      $zip->addFile($zipTmpDir.'customXml/itemProps1.xml', 'customXml/itemProps1.xml');
    }

    $zip->addEmptyDir('docProps');
    $zip->addFile($zipTmpDir.'docProps/app.xml', 'docProps/app.xml');
    $zip->addFile($zipTmpDir.'docProps/core.xml', 'docProps/core.xml');
    $zip->addEmptyDir('word');
    $zip->addEmptyDir('word/_rels');
    $zip->addFile($zipTmpDir.'word/_rels/document.xml.rels', 'word/_rels/document.xml.rels');
    $zip->addFile($zipTmpDir.'word/_rels/settings.xml.rels', 'word/_rels/settings.xml.rels');
    $zip->addEmptyDir('word/theme');
    $zip->addFile($zipTmpDir.'word/theme/theme1.xml', 'word/theme/theme1.xml');
    $zip->addFile($zipTmpDir.'word/document.xml', 'word/document.xml');
    $zip->addFile($zipTmpDir.'word/endnotes.xml', 'word/endnotes.xml');
    $zip->addFile($zipTmpDir.'word/fontTable.xml', 'word/fontTable.xml');
    $zip->addFile($zipTmpDir.'word/footer1.xml', 'word/footer1.xml');
    $zip->addFile($zipTmpDir.'word/footer2.xml', 'word/footer2.xml');
    $zip->addFile($zipTmpDir.'word/footnotes.xml', 'word/footnotes.xml');
    $zip->addFile($zipTmpDir.'word/numbering.xml', 'word/numbering.xml');
    $zip->addFile($zipTmpDir.'word/settings.xml', 'word/settings.xml');
    $zip->addFile($zipTmpDir.'word/styles.xml', 'word/styles.xml');
    $zip->addFile($zipTmpDir.'word/stylesWithEffects.xml', 'word/stylesWithEffects.xml');
    $zip->addFile($zipTmpDir.'word/webSettings.xml', 'word/webSettings.xml');
    $zip->addFile($zipTmpDir.'[Content_Types].xml', '[Content_Types].xml');

    $zip->close();
  }

    //ダウンロードファイル名
    $DLfilename = $ScenarioTitle . ".docx";

    //ストリームに出力
    header('Content-Type: application/octet-stream');

    //ファイル名の文字コード変換
    if (preg_match('/\bMSIE\b|\bSafari [12345]\b/', getenv('HTTP_USER_AGENT'))) {
      header('Content-Disposition: attachment; filename="' .
        mb_convert_encoding($DLfilename, 'Shift_JIS', 'UTF-8') . '"');
    }else {
      header('Content-Disposition: attachment; filename*=UTF-8\'\'' . rawurlencode($DLfilename));
    }
    //ファイルサイズ
    header('Content-Length: '.filesize($zipTmpDir.$zipFileName));

    //出力用バッファをクリア
    ob_end_clean();
    //出力（ダウンロード）
    readfile($zipTmpDir.$zipFileName);
    //一時保存ファイルを削除
    unlink($zipTmpDir.$zipFileName);

    //シナリオtmpディレクトリ削除
    fncRecursiveRemoveDir($zipTmpDir);

}
//--------------------------------------------------------------------------------
//    fncRecursiveCopyDir($src, $dst)
//      $src  コピー元
//      $dst  コピー先
//
//    $dir内の全てを上書きｺﾋﾟｰ
//    http://wakichoko.doorblog.jp/archives/13474955.html
//--------------------------------------------------------------------------------
function fncRecursiveCopyDir($src, $dst){
  //ｺﾋﾟｰ先があれば削除する
  if (file_exists($dst)) {
    fncRecursiveRemoveDir($dst);
  }

  //ｺﾋﾟｰ元があればｺﾋﾟｰを始める
  if (is_dir($src)){
    //ｺﾋﾟｰ先を作る
    mkdir($dst);
    //ｺﾋﾟｰ元をﾘｽﾄする
    $files = scandir($src);
    foreach ($files as $file) {
      if (($file != ".") && ($file != "..")){
        //回帰的にｺﾋﾟｰする
        fncRecursiveCopyDir("$src/$file", "$dst/$file");
      }
    }
  }else if (file_exists($src)) {
    //通常にｺﾋﾟｰする
    copy($src, $dst);
  }
}
//--------------------------------------------------------------------------------
//    fncRecursiveRemoveDir($dir)
//
//    $dir内の全てを削除
//
//    元ネタは、回帰的な削除
//    http://www.php.net/manual/ja/function.unlink.php#87045
//                        by Jon Hassall
//--------------------------------------------------------------------------------
function fncRecursiveRemoveDir($dir) {
  //$dir が存在しないときは何もしない
  if(!$dh = @opendir($dir)){
    return;
  }
  //無くなるまで繰り返す
  while (false !== ($obj = readdir($dh))) {
    if($obj == '.' || $obj == '..') {
      continue;
    }
    //$dir内のﾃﾞｨﾚｸﾄﾘ削除（回帰的な削除）
    if (!@unlink($dir . '/' . $obj)) {
      fncRecursiveRemoveDir($dir.'/'.$obj);
    }
  }
  //一旦閉じる
  closedir($dh);
  //$dirを削除
  @rmdir($dir);

  return;
}


// -----------------------------------------------------------
?>
